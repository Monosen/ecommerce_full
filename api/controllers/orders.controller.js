...
exports.addProductToCart = catchAsync(async (req, res, next) => {
  ...
  if (cart) {
    ...
    const updatedTotalPrice = +cart.totalPrice + +filteredObj.quantity * +productExists.price
...
exports.updateProductCart = catchAsync(async (req, res, next) => {
  ...
  if (newQuantity === 0) {
    updatedTotalPrice = +userCart.totalPrice - +productInCart.quantity * +productInCart.price

    // Update quantity to product in cart
    await productInCart.update({ quantity: 0, status: 'removed' })
  } else if (newQuantity > +productInCart.quantity) {
    // New items were added
    updatedTotalPrice = +userCart.totalPrice + (newQuantity - +productInCart.quantity) * +productInCart.price

    // Update quantity to product in cart
    await productInCart.update({ quantity: newQuantity })
  } else if (newQuantity < +productInCart.quantity) {
    // Items were removed from the cart
    updatedTotalPrice = +userCart.totalPrice - (+productInCart.quantity - newQuantity) * +productInCart.price
...
exports.getOrderById = catchAsync(async (req, res, next) => {
  ...
  if (!order) {
    return next(new AppError('Not product in cart', 404))
  }
